...
<!-- TODO: 
  ...
  - Brown / tan background for osrs theme?
...
# Enhanced Crystal Weapon Seed <img src='images/Enhanced_crystal_weapon_seed_detail.png' style='height:1em; vertical-align:top;'>
...
$$ X \sim \text{Geom}(p) \implies Q_X(q) = F_X^{-1}(q) = \left\lceil \frac{\ln(1-q)}{\ln(1-p)} \right\rceil - 1$$ 
...
$$N \sim \text{Geom}(p) \implies Q_N(q) = F_N^{-1}(q) = F_X^{-1}(q) + 1 = \left\lceil \frac{\ln(1-q)}{\ln(1-p)} \right\rceil$$
...
# Crystal Armour Seed <img src='images/Crystal_armour_seed_detail.png' style='height:1em; vertical-align:top;'>
...
$$X \sim \text{NegativeBinomial}(r,p) \implies E[X] = \frac{r(1-p)}{p}$$ **Where:**
...
$$N \sim \text{NegativeBinomial}(r,p) \implies E[N] = \frac{r(1-p)}{p} + r = \frac{r}{p}$$ And indeed, plugging in our number of successes needed and probability provides us with the number of KC we can expect it to take to receive 6 Crystal armour seeds:
...
$$N \sim \text{NegativeBinomial}(r,p) \implies F_N^{-1}(q) = \min \left\{ n \in \mathbb{Z} \mid n \ge r \text{ and } \sum_{i=r}^{n} \binom{i-1}{r-1} p^r (1-p)^{i-r} \ge q \right\}$$ Unlike the geometric distribution, the cumulative distribution function (CDF) for the negative binomial distribution involves a **moving summation of combinations**. Because our target variable $n$ is the upper limit of this summation, there is no closed-form algebraic solution to isolate $n$. We cannot simply 'solve for $n$' using standard algebra as we have done before.
...
img_tag <- "<img src='images/Crystal_armour_seed_detail.png' width='10%' style='display:inline-block; margin:2px;'>"
